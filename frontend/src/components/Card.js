...
function Card({movie, isMore}) {
  return(
    <Link to={`/movie/${movie.id}` } target="_blank">
      <div className='w-full green-pink-gradient p-[3px] rounded-[20px] shadow-card'>
        <div className={`flex-col flex gap-2 items-center pb-5 bg-black rounded-[20px] ${isMore ? 'border-solid' : 'border-none'}`} style={{ width: '300px', height: '440px' }}>
